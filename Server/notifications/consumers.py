...
class NotificationConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        try:
            ...
            if self.current_user:
                logger.info(
                    f"Notification WebSocket authenticated: "
                ...
                if self.channel_layer:
                    ...
                    logger.info(
                        f"Added to notification group: {self.group_name}"
                    )

                await self.accept()
                logger.info(
                    f"Notification WebSocket connection accepted for "
                    f" user_id={self.current_user.id}"
                )
            else:
            ...
            logger.error(
                f"Error in notification connect: {str(e)}", exc_info=True
            )
            await self.close()
    ...
    async def disconnect(self, close_code):
        try:
            logger.info(
                f"Notification WebSocket disconnection with code {close_code}"
            )
            if hasattr(
                self, 'current_user'
            ) and self.current_user and hasattr(self, 'group_name'):
                await self.channel_layer.group_discard(
                    self.group_name, self.channel_name
                )
        except Exception as e:
            logger.error(
                f"Error in notification disconnect: {str(e)}", exc_info=True
            )
    ...
    async def receive(self, text_data):
        try:
            ...
            logger.error(
                f"Error in notification receive: {str(e)}", exc_info=True
            )
